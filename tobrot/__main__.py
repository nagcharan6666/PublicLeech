...
if __name__ == "__main__" :
    ...
    incoming_message_handler = MessageHandler(
        incoming_message_f,
        filters=Filters.command(["leech@Alpacino_The_bot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(incoming_message_handler)
    #
    incoming_youtube_dl_handler = MessageHandler(
        incoming_youtube_dl_f,
        filters=Filters.command(["ytdl@Alpacino_The_bot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(incoming_youtube_dl_handler)
    #
    status_message_handler = MessageHandler(
        status_message_f,
        filters=Filters.command(["status@Alpacino_The_bot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    help_text_handler = MessageHandler(
        help_message_f,
        filters=Filters.command(["help@Alpacino_The_bot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    save_thumb_nail_handler = MessageHandler(
        save_thumb_nail,
        filters=Filters.command(["savethumbnail@Alpacino_The_bot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(save_thumb_nail_handler)
    #
    clear_thumb_nail_handler = MessageHandler(
        clear_thumb_nail,
        filters=Filters.command(["clearthumbnail@Alpacino_The_bot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
